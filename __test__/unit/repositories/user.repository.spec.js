...
describe('User Repository Unit Test', () => {
  ...
  test('findUserByEmail Method', async () => {
    const mockReturn = { email: 'test@example.com' };
    ...
    await userRepository.findUserByEmail(findUserEmail.email);
  ...
  test('getUserPoint Method', async () => {
    const mockReturn = 'getUserPoint Start';
    mockPrisma.user.findFirst.mockReturnValue(mockReturn);
    const getUserPointParams = {
      userId: 1,
      point: true,
    };
    const getUserPointData = await userRepository.getUserPoint(
      getUserPointParams.userId,
      getUserPointParams.point,
    );

    expect(getUserPointData).toEqual(mockReturn);
    expect(mockPrisma.user.findFirst).toHaveBeenCalledTimes(1);
    expect(mockPrisma.user.findFirst).toHaveBeenCalledWith({
      where: {
        userId: getUserPointParams.userId,
      },
      select: {
        point: getUserPointParams.point,
      },
    });
  });
});
